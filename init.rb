...
ActiveRecord::Base.class_eval do
  def self.has_settings options = {}
    ...
    class_eval do
      ...
      send scope_method, :with_settings, 
      ...
      send scope_method, :with_settings_for, 
        lambda { |var| { :joins => "JOIN settings ON (settings.target_id = #{self.table_name}.#{self.primary_key} AND
                                                      settings.target_type = '#{self.base_class.name}') AND
                                                      settings.var = '#{var}'" } }
                                                               
      send scope_method, :without_settings, 
                         :joins => "LEFT JOIN settings ON (settings.target_id = #{self.table_name}.#{self.primary_key} AND
                                                           settings.target_type = '#{self.base_class.name}')",
                         :conditions => 'settings.id IS NULL'
                                     
      send scope_method, :without_settings_for, 
        lambda { |var| { :joins => "LEFT JOIN settings ON 
